...
class SwitchTests(TestCase):
    def test_switch_active(self):
        ...
        request = get()
        assert waffle.switch_is_active(request, switch.name)

    def test_switch_inactive(self):
        switch = Switch.objects.create(name='myswitch', active=False)
        request = get()
        assert not waffle.switch_is_active(request, switch.name)
    ...
    def test_undefined(self):
        request = get()
        assert not waffle.switch_is_active(request, 'foo')

    @override_settings(WAFFLE_SWITCH_DEFAULT=True)
    def test_undefined_default(self):
        request = get()
        assert waffle.switch_is_active(request, 'foo')
